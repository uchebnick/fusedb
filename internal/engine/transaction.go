...
var (
	ErrEmptyTransaction     = errors.New("fusedb: transaction has no mutations")
	ErrTooManyMutations     = errors.New("fusedb: transaction has too many mutations")
	ErrDuplicateMutationKey = errors.New("fusedb: transaction mutates a key more than once")
	ErrIdempotencyConflict  = errors.New("fusedb: idempotency key was already used for different mutations")
)
...
func (db *DB) commitMutationsLocked(mutations []Mutation) (bool, error) {
	...
	for _, mutation := range mutations {
		...
		if err != nil {
			...
			db.applyMu.RUnlock()
			failure := fmt.Errorf("fusedb: apply durable transaction: %w", err)
...
func validateTransaction(conditions []Condition, mutations []Mutation) error {
	...
	for _, condition := range conditions {
		...
		if condition.kind != conditionAbsent && condition.kind != conditionPresent {
			return errors.New("fusedb: invalid transaction condition")
		}
	}
	...
	for _, mutation := range mutations {
		...
		default:
			return errors.New("fusedb: invalid transaction mutation")
		}

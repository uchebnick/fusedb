...
var (
	ErrDatabaseLocked = errors.New("oneleafdb: database directory is locked")
	// ErrCorruption wraps any failed integrity invariant found by Verify.
	ErrCorruption = errors.New("oneleafdb: corruption detected")
	...
	ErrSchedulerModelPersistence = errors.New("oneleafdb: scheduler model persistence failed")
	...
	ErrFormatMismatch         = errors.New("oneleafdb: format descriptor does not match persisted files")
	...
	ErrKeyTooLarge    = errors.New("oneleafdb: key exceeds maximum size")
	ErrValueTooLarge  = errors.New("oneleafdb: value exceeds maximum size")
)
...
func OpenDB(opts DBOptions) (*DB, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("oneleafdb: lock database directory: %w", err)
	}
	keepLock := false
	defer func() {
		if !keepLock {
	...
	case formatErr == nil:
		if err := dbformat.CheckCompatible(formatDescriptor); err != nil {
			return nil, fmt.Errorf("oneleafdb: incompatible database format: %w", err)
		}
	case errors.Is(formatErr, os.ErrNotExist):
		formatDescriptor = dbformat.Current()
	default:
		return nil, fmt.Errorf("oneleafdb: load database format: %w", formatErr)
	}
	...
	if formatPresent {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("oneleafdb: inspect dictionary group catalog: %w", err)
		}
	}
	...
	if activeDictionary == nil && loaded != nil {
		...
		if inferErr != nil {
			closeRegistry(registry)
			return nil, fmt.Errorf("oneleafdb: infer legacy active dictionary: %w", inferErr)
		}
		if found {
			if err := catalog.EnsureNextAfter(inferredID); err != nil {
				closeRegistry(registry)
				return nil, fmt.Errorf("oneleafdb: migrate legacy dictionary allocator: %w", err)
			}
			if _, err := catalog.Publish(DefaultDictionaryGroupID, inferredID); err != nil {
				closeRegistry(registry)
				return nil, fmt.Errorf("oneleafdb: migrate legacy dictionary policy: %w", err)
			}
			activeDictionary, err = registry.Load(inferredID)
			if err != nil {
				closeRegistry(registry)
				return nil, fmt.Errorf("oneleafdb: load migrated dictionary %d: %w", inferredID, err)
			}
			compressionKind = segment.CompressionLZ4Dict
		}
	}
	...
	if err := trainingConfig.validate(); err != nil {
		closeRegistry(registry)
		return nil, fmt.Errorf("oneleafdb: dictionary training config: %w", err)
	}
	...
	if err := reconcileOrphanSegments(fs, opts.Dir, leafTree.Manifest()); err != nil {
		_ = leafTree.Close()
		closeRegistry(registry)
		return nil, fmt.Errorf("oneleafdb: reconcile orphan segments: %w", err)
	}
	if err := leafTree.RebalanceDictionaryGroups(); err != nil {
		_ = leafTree.Close()
		closeRegistry(registry)
		return nil, fmt.Errorf("oneleafdb: rebalance dictionary groups: %w", err)
	}
	if !formatPresent {
		...
		if err := leafTree.SaveManifest(); err != nil {
			_ = leafTree.Close()
			closeRegistry(registry)
			return nil, fmt.Errorf("oneleafdb: migrate manifest to v%d: %w", manifest.CurrentFormatVersion, err)
		}
	}
	...
	if !formatPresent || formatDescriptor.Epoch < dbformat.CurrentEpoch {
		...
		if err := dbformat.SaveCurrent(fs, opts.Dir); err != nil {
			...
			_ = leafTree.Close()
			closeRegistry(registry)
			return nil, fmt.Errorf("oneleafdb: publish database format: %w", err)
		}
		formatDescriptor = dbformat.Current()
	...
	if err != nil {
		...
		_ = leafTree.Close()
		closeRegistry(registry)
		return nil, fmt.Errorf("oneleafdb: create scheduler: %w", err)
	}
...
func (db *DB) Backup(ctx context.Context, archivePath string) (BackupReport, error) {
	...
	if db == nil || db.background == nil || db.tree == nil {
		return BackupReport{}, errors.New("oneleafdb: database is not open")
	}
	if db.closed.Load() {
		return BackupReport{}, errors.New("oneleafdb: database is closed")
	}
	...
	if err != nil {
		return BackupReport{}, fmt.Errorf("oneleafdb: resolve backup path: %w", err)
	}
	if inside {
		return BackupReport{}, errors.New("oneleafdb: backup archive must be outside the database directory")
	}
...
func (db *DB) backupNow(ctx context.Context, archivePath string) (BackupReport, error) {
	...
	if !db.schedulerModelDisabled {
		if err := db.persistSchedulerModelNow(ctx); err != nil {
			return BackupReport{}, fmt.Errorf("oneleafdb: persist scheduler model for backup: %w", err)
		}
	}
	formatData, err := disk.ReadFileLimited(db.fs, dbformat.FileName(db.dir), limits.MaxFormatBytes)
	if err != nil {
		return BackupReport{}, fmt.Errorf("oneleafdb: read database format for backup: %w", err)
	}
	...
	if err != nil {
		return BackupReport{}, fmt.Errorf("oneleafdb: encode backup manifest: %w", err)
	}
	...
	if err != nil {
		return BackupReport{}, fmt.Errorf("oneleafdb: read dictionary group catalog: %w", err)
	}
	...
	if !db.schedulerModelDisabled {
		...
		if err != nil {
			return BackupReport{}, fmt.Errorf("oneleafdb: read scheduler model for backup: %w", err)
		}
	...
	if err != nil {
		return BackupReport{}, fmt.Errorf("oneleafdb: write backup: %w", err)
	}
	return BackupReport{
...
func (db *DB) Verify(ctx context.Context) (VerifyReport, error) {
	...
	if db == nil || db.background == nil || db.tree == nil {
		return VerifyReport{}, errors.New("oneleafdb: database is not open")
	}
	if db.closed.Load() {
		return VerifyReport{}, errors.New("oneleafdb: database is closed")
	}
	if err := db.backgroundErr(); err != nil {
...
func (db *DB) verifyNow(ctx context.Context) (VerifyReport, error) {
	...
	if err := db.wal.Sync(); err != nil {
		return VerifyReport{}, fmt.Errorf("oneleafdb: verify sync wal: %w", err)
	}
...
func (db *DB) Merge() error {
	if db == nil || db.background == nil {
		return errors.New("oneleafdb: database is not open")
	}
	if db.closed.Load() {
		return errors.New("oneleafdb: database is closed")
	}
	if err := db.backgroundErr(); err != nil {
		return err
	}
	...
	if !db.background.Submit(job) {
		return errors.New("oneleafdb: scheduler is closed")
	}
	return <-done
}
...
func (db *DB) checkpoint() error {
	...
	if err := db.tree.SetAppliedSeq(watermark); err != nil {
		return fmt.Errorf("oneleafdb: set applied sequence: %w", err)
	}
	...
	db.closeOnce.Do(func() {
		...
		if !db.schedulerModelDisabled && db.background != nil {
			if modelErr := db.persistSchedulerModelNow(context.Background()); modelErr != nil {
				err = fmt.Errorf("oneleafdb: persist scheduler model on close: %w", modelErr)
			}
		}
